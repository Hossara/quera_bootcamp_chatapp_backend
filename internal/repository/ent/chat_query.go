...
func (_q *ChatQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Chat, error) {
	...
	if query := _q.withCreator; query != nil {
		if err := _q.loadCreator(ctx, query, nodes, nil,
			func(n *Chat, e *User) { n.Edges.Creator = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withMembers; query != nil {
		if err := _q.loadMembers(ctx, query, nodes,
			func(n *Chat) { n.Edges.Members = []*ChatMember{} },
			func(n *Chat, e *ChatMember) { n.Edges.Members = append(n.Edges.Members, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ChatQuery) loadCreator(ctx context.Context, query *UserQuery, nodes []*Chat, init func(*Chat), assign func(*Chat, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Chat)
	for i := range nodes {
		if nodes[i].user_created_chats == nil {
			continue
		}
		fk := *nodes[i].user_created_chats
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_created_chats" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *ChatQuery) loadMembers(ctx context.Context, query *ChatMemberQuery, nodes []*Chat, init func(*Chat), assign func(*Chat, *ChatMember)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Chat)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.ChatMember(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(chat.MembersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.chat_members
		if fk == nil {
			return fmt.Errorf(`foreign-key "chat_members" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "chat_members" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
